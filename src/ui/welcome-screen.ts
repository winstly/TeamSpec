/**
 * Animated welcome screen for the TeamSpec workflow setup.
...
function getWelcomeText(): string[] {
  return [
    chalk.white.bold('Welcome to TeamSpec'),
    chalk.dim('AI agent team collaboration framework'),
    '',
    chalk.white('This setup will configure:'),
    chalk.dim('  • Agent Skills for your team'),
    chalk.dim('  • Structured multi-agent workflows'),
    '',
    chalk.white('Quick start after setup:'),
    `  ${chalk.yellow('teamspec status')} ${chalk.dim('Check workspace health')}`,
    `  ${chalk.yellow('teamspec update')}   ${chalk.dim('Refresh skills')}`,
    '',
...
export async function showWelcomeScreen(): Promise<void> {
  ...
  if (!canAnimate()) {
    // Fallback: show static welcome — use the complete T frame (last frame)
